...
'''
Not a lot "fake" about it, this module is essentially a direct copy
of "events.py" from evdev, except that it imports from fake_ecodes
instead of evdev.ecodes.
...
from fake_ecodes import keys, KEY, SYN, REL, ABS, FF, MSC, EV_KEY, EV_REL, EV_ABS, EV_SYN, EV_FF, EV_MSC
...
class RelEvent(object):
	...
	def __str__(self):
		...
		return msg.format(self.event.timestamp(), REL[self.event.code], self.event.value)
...
class AbsEvent(object):
	...
	def __str__(self):
		...
		return msg.format(self.event.timestamp(), ABS[self.event.code], self.event.value)
...
class FFEvent(object):
	'''An event generated by force feedback stuff.'''
	...
	def __str__(self):
		msg = 'force feedback event at {:f}, {} '
		return msg.format(self.event.timestamp(), FF.get(self.event.code, f"UNKNOWN ({self.event.code}, {self.event.value})"))
...
class MSCEvent(object):
	'''A miscellaneous event which represents miscellany.'''
	__slots__ = 'event'
	...
	def __str__(self):
		msg = 'miscellaneous event at {:f}, {} ({})'
		return msg.format(self.event.timestamp(), MSC[self.event.code], self.event.value)
...
__all__ = ('InputEvent', 'KeyEvent', 'RelEvent', 'SynEvent',
		   'AbsEvent', 'FFEvent', 'event_factory', 'categorize')
